...
export default class DatePicker {
  ...
  constructor(options) {
    ...
    this.dayCallback = options.dayCallback;
  ...
  selectDay(item, dayClass) {
    ...
    this.callback(this.year, this.month, this.day, dayClass);

  }

  compareDate(item) {
    // console.log(item, this.context.targetRange)
    const context = this.context;
    let start = parseInt(context.targetRange[0].split('-').join(''));
    let end = parseInt(context.targetRange[1].split('-').join(''));
    if (start > end) {
  ...
  switchPanel(h, type) {
    ...
    const month = monthPanel.call(this, h);

    const day = dayPanel.call(this, h);

    const year = yearPanel.call(this, h);
